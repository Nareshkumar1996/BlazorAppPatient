...
namespace Healthware.Assist.Core.Extensions
{
    public static class StringExtensions
    {
      ...
      separator
        ...
        public static string SmartTrim(this string value)
        {
            if (value == null)
                return null;
            return value.Trim();
        ...
        public static string FirstXXXCharacters(this string subject, int numberOfCharacters)
        {
            if (string.IsNullOrEmpty(subject))
                return string.Empty;

            return subject.Length > numberOfCharacters ? "{0} [...]".FormatWith(subject.Substring(0,numberOfCharacters)) : subject;
        }

        public static string FormatWith(this string string_to_be_formatted, params object[] args)
        {
            return string.Format(string_to_be_formatted, args);
        }

        public static bool EqualToIgnoringCase(this string left_string, string right_string)
        {
            return string.Compare(left_string, right_string, true) == 0;
        }
        ...
        public static string OrEmptyString(this string actual)
        {
            return actual ?? string.Empty;
        }

        public static int CountOccurrencesOf(this string string_to_check, string string_to_find)
        {
            var r = new Regex(string_to_find, RegexOptions.IgnoreCase);
            var ma = r.Matches(string_to_check);
            return ma.Count;
        }

        public static string ReplaceSpacesWithUnderscores(this string stringToPerformReplacementOn)
        {
            return stringToPerformReplacementOn.Replace(" ", "_");
        }
        
        public static bool IsIso8601ExtenededFormat(this string date)
        {
            var isValidMatch = Regex.IsMatch(date, @"(?<YEAR>\d{4})-(?<MONTH>0[1-9]|1[012])-(?<DAY>0[1-9]|[12][0-9]|3[01])");

            DateTime necessaryEvil;
            return isValidMatch && DateTime.TryParse(date, out necessaryEvil);
        }

        public static bool IsIso8601ExtenededFormatOrEmptyString(this string date)
        {
            if (string.IsNullOrEmpty(date))
                return true;

            return IsIso8601ExtenededFormat(date);
        }

        public static string AsCommaDelimited(this IEnumerable<string> list_of_strings)
        {
            var builder = new StringBuilder();
            list_of_strings.Each(x => builder.AppendFormat(" {0},",x));
            if (builder.Length > 2)
            {
                builder.Remove(0, 1);
        ...
        public static string MakeFilenameSafe(this string subject)
        {
            foreach (var unsafeChar in Path.GetInvalidFileNameChars())
                subject = subject.Replace(unsafeChar, '_');
            return subject;
        }

        public static string And(this string subject,string string_to_attach)
        {
            return "{0},{1}".FormatWith(subject, string_to_attach);
        }

        public static string ToCommaDeliminatedWithSpace(this string[] subject)
        {
        ...
        public static string ToCommaDeliminated(this string[] subject)
        {
            StringBuilder result = new StringBuilder();
            foreach (var item in subject)
            {
        ...
        public static string WithInitialCapital(this string subject)
        {
            return subject.isNullOrEmpty() ? "" : subject.Substring(0, 1).ToUpper() + subject.Substring(1);
        }

        public static bool Contains(this string source, string toLookup, StringComparison stringComparison)
        {
            return source?.IndexOf(toLookup, stringComparison) >= 0;
        }
